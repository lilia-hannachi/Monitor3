...
function generateDonut(dataset, divName, title)
{
	...
	{
		generateDonut1(dataset, divName, title);
	} 
}

function generateDonut1(dataset, divName, title)
{
      d3.select("#"+divName).select('div').remove();
      ...
      var charts = d3.select("#"+divName)
                  ...
                  /* responsive SVG needs these 2 attributes and no width and height attr */
                   ...
                   .attr("viewBox", "0 0 450 450") /* class to make it responsive */
                   .classed("svg-content-responsive", true);      

      var chart_m = donut_width / dataset.length / 2 * 0.14;
      var chart_r = donut_width / dataset.length / 3 * 0.85;
  ...
  var donut = charts.selectAll('.donut').data(dataset).enter().append('svg:svg').attr('width', donut_width+80).attr('height', donut_height)
                    .append('svg:g').attr('class', function(d, i) {return 'donut type' + i;}).attr('transform', 'translate(' + (chart_r+chart_m+80) + ',' + (chart_r+chart_m) + ')');
